		...
		<!-- 悬赏管理 -->
				<div class="company-personal-wrap cpmpany-shadow" style="padding: 30px 30px 0 30px !important;">
				  	<div class="reward-manage">
					    <div class="clearfix  reward-header">
					    	...
					    	<div class="fl">
							      <p style="color:#4a4c52;font-size: 14px;">发布赏金计划，鼓励更多的白帽子对您的项目进行持续检测和发现漏洞，帮您掌握安全主动权。计划内容可以随时更改。</p><br />
		            			  <router-link to="/Bounty" target="_blank">[关于赏金计划]</router-link>
					    	</div>
            			    <div class="fr" style="text-align: right;">
            				  	<router-link :to="{path:'/companyTwoCons',query:{company:mycompany}}" target="_blank"><el-button class="btnshadow wrapBtnw" style="border: 0;">预览</el-button></router-link>
            			  		<el-button  class="btnshadow wrapBtn" style="margin-left: 10px;"  type="primary" @click="edit_CZdjfbbtn"  :disabled="isDisable">发布</el-button><br />
			...
			<el-card  shadow="hover" class="rewardcon" style="margin-top: 30px;background: #ffffff;padding: 40px 20px 20px 0;"> 
					    	<el-tabs :tab-position="tabPosition"   v-model="activeName"  @tab-click="handleClick">
							    <el-tab-pane label="1.奖励计划" name="first">
						    		<div>
						    			<div class="rewardConhead">漏洞的赏金基于漏洞的严重程度。评判参考 <router-link to="/rewardReference" target="_blank">[参考模板]</router-link></div>
										...
										<div class='fr'>
					  						<el-button type="primary" class="btnshadow wrapBtn"  @click="rewardNext1" :plain="true">下一步</el-button>
										</div>
						    		</div>
							    </el-tab-pane>
							    <el-tab-pane label="2.测试范围" v-if="this.dataCsfw==''" disabled name="second">
							    	<div class="rewardConhead">规定安全人员可以对您哪些域名/应用进行测试；哪些域名/应用不可以测试。评判参考 <router-link to="/rewardReference" target="_blank">[参考模板]</router-link></div>
							    	<div class="reward-manage-item clearfix">
							    		<div class="reward-plan-edit">
									      	<el-input type="textarea" autosize  placeholder="请输入内容" v-model="dataCsfw"></el-input>
									    </div>
								    </div>
								    <div class='fr'>
											<el-button class="comColor btnshadow wrapBtnw" @click='rewardLast2' >上一步</el-button>
					  						<el-button type="primary" class="btnshadow wrapBtn" @click="rewardNext2" :plain="true">下一步</el-button>
										</div>
							    </el-tab-pane>
							    <el-tab-pane label="2.测试范围" v-if="this.dataCsfw!=''" name="second">
							    	<div class="rewardConhead">规定安全人员可以对您哪些域名/应用进行测试；哪些域名/应用不可以测试。评判参考 <router-link to="/rewardReference" target="_blank">[参考模板]</router-link></div>
							    	<div class="reward-manage-item clearfix">
							    		<div class="reward-plan-edit">
									      	<el-input type="textarea" autosize  placeholder="请输入内容" v-model="dataCsfw"></el-input>
							    ...
							    <el-tab-pane label="3.漏洞评级" v-if="!this.dataVulSer " disabled name="third">
							    	<div class="rewardConhead">定义漏洞的严重程度。 评判参考 <router-link to="/rewardReference"  target="_blank">[参考模板]</router-link></div>
							    	<div class="reward-manage-item clearfix">
									    <div class="reward-plan-edit">
									     		<p>低危</p>
									      		<el-input  type="textarea" autosize  placeholder="请输入内容"  v-model='dataVulLow'></el-input>
									    </div>
									    <div class="reward-plan-edit">
									    		<p>中危</p>
									      		<el-input type="textarea"  autosize  placeholder="请输入内容"  v-model='dataVulMid'></el-input>
									    </div>
									    <div class="reward-plan-edit">
									    		<p>高危</p>
									      		<el-input  type="textarea" autosize placeholder="请输入内容"   v-model='dataVulHigh'></el-input>
									    </div>
									    <div class="reward-plan-edit">
									    		<p>严重</p>
									      		<el-input  type="textarea" autosize  placeholder="请输入内容"  v-model='dataVulSer'></el-input>
									    </div>
									</div>
							   		 <div class='fr'>
										<el-button class="comColor btnshadow wrapBtnw"  @click='rewardLast3' >上一步</el-button>
				  						<el-button type="primary"class="btnshadow wrapBtn"  @click="rewardNext3" :plain="true">下一步</el-button>
									</div>
							    </el-tab-pane>
							    <el-tab-pane label="3.漏洞评级" v-if="this.dataVulSer" name="third">
							    	<div class="rewardConhead">定义漏洞的严重程度。 评判参考 <router-link to="/rewardReference"  target="_blank">[参考模板]</router-link></div>
							    	<div class="reward-manage-item clearfix">
									    <div class="reward-plan-edit">
									     		<p>低危</p>
							    ...
							    <el-tab-pane label="4.注意事项" v-if="!this.dataVulSer"  disabled name="fourth">
							    	<div class="rewardConhead">需要补充说明的事情<span style="color:#F00;font-size: 14px;color: #b4bdc2;">（非必填项）</span>评判参考 <router-link to="/rewardReference" target="_blank">[参考模板]</router-link></div>
							        <div class="reward-manage-item clearfix">
								        <div class="reward-plan-edit">
								    ...
								    <div class='fr'>
										<el-button class="comColor btnshadow wrapBtnw"  @click='rewardLast4' >上一步</el-button>
				  						<el-button type="primary" class="btnshadow wrapBtn" @click="rewardNext4"  :plain="true">完成</el-button>
									</div>
							    </el-tab-pane>
							    <el-tab-pane label="4.注意事项"  v-if="this.dataVulSer"  name="fourth">
							    	<div class="rewardConhead">需要补充说明的事情<span style="color:#F00;font-size: 14px;color: #b4bdc2;">（非必填项）</span>评判参考 <router-link to="/rewardReference" target="_blank">[参考模板]</router-link></div>
							        <div class="reward-manage-item clearfix">
								        <div class="reward-plan-edit">
									      		<el-input type="textarea" autosize  placeholder="请输入内容" v-model="remarks"></el-input>
			...
			<!-- 余额不足 -->
				<el-dialog
					 ...
					 <div class="common-pop-up">
					 	<h3>您需要存储担保金发布漏洞赏金计划</h3>
					 	 <p style="color:#4a4c52;padding-top: 10px;">为保障您能够奖励白帽子（漏洞提交者），请先存储不低于<span>{{serPriceEnd}}</span>Ether（单个严重漏洞的最高赏金）到您的担保金账户</p>  <br /><br />
					 	 担保金剩余：<span>{{$store.state.CSuser.user.balance}}</span>Ether，余额不足 &nbsp;&nbsp;<el-button  type="primary" class='fr btnshadow wrapBtn' @click="dialogVisibleCZbtn"> 担保金充值 </el-button><br>
					      <p></p>
					    </div>
				</el-dialog>
				<!--充值弹窗-->
				<el-dialog
					 ...
					 <div style="width: 720px;text-align: center;">
						    <div class="apply-safeguards-pop-up common-pop-up">
						      <p class="apply-title" style="margin-bottom: 30px;">支付担保金
						      	<el-tooltip placement="bottom">
								  <div slot="content">
								  		<div>
									        <p class="apply-info-text" style="font-weight: 600;">担保金说明</p>
									        <p class="apply-info-text">• 担保金是预存储在DVP的漏洞赏金；</p>
									        <p class="apply-info-text">• 发布赏金计划需存储的担保金不能低于单个严重漏洞的最高奖励值。</p>
									        <p class="apply-info-text">•担保金支持随时退款。</p>
									        <p class="apply-info-text">•充值遇到问题请邮件联系DVP客服</p>
						      ...
						      <div class="apply-address">支付地址：0xd5bAA94e09C4a1C6613A35c485604F507b368517</div>
						      <div class="apply-scan">
						        <img src="/static/image/eth_code.png">
						        <p>扫码支付</p>
						      </div>
						      <input type="text" placeholder="请输入交易号" id="order" class="form-control-inp" style="width: 80%;margin-bottom: 10px;" v-model="orderPrice">
						      <div style="clear: both;"></div>
						      <el-button type="primary" class="btnshadow wrapBtn" @click="rewardBtn">支付完成</el-button>
						    </div>
					</div>
				</el-dialog>
				<!-- 充值成功 -->
				<el-dialog
				  ...
				  width="500px">
				  	<div class="dialogTXCon">
				  		<p style="margin-bottom: 30px;">DVP将在15分钟内完成审核，请稍等。</p>
				  		<el-button type="primary" class="btnshadow wrapBtn" @click="VulRLpayDBJSucsBtn">确定</el-button>
					</div>
			    </el-dialog>
				<!-- 支付担保金弹窗 -->
				<el-dialog
				...
				<el-dialog
					 ...
					 <div class="common-pop-up ac">
					 	<div class="ac">发布悬赏前，请完善企业简介和LOGO等信息</div>
					 	<router-link to="/combox/company/comMember"><el-button  type="primary" style="margin-top: 20px;" class="btnshadow wrapBtn">完善信息</el-button></router-link>
					      <p></p>
					    </div>
				</el-dialog>
				<!-- 发布成功 -->
				...
				<el-dialog
					 ...
					 <div class="common-pop-up ac">
					 	...
					 	<el-button  type="primary" style="margin-top: 20px;" class="btnshadow wrapBtn"  @click="otherRewardBtn" >继续编辑</el-button>
					      <p></p>
					    </div>
				</el-dialog>
				<!-- 身份审核 -->
				<el-dialog
					  ...
					  width="400px">
					 <div class="common-pop-up ac">
					 	<div class="ac">您已成功注册DVP账号，请等待身份审核。</div>
					      <p></p>
					    </div>
				</el-dialog>
		  </div>
