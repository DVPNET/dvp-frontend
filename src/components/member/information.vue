		                ...
		                <div class="form-input">
					        ...
					        <div class="my-info-edit-item clearfix">
					        	...
					        	<div class="tags-select fr">
					        		<el-tag  :key="tag"  v-for="tag in skillTag"  size="medium"  closable  :disable-transitions="false" @close="handleClosebq(tag)">{{tag}} </el-tag>
					        	</div>
					        	<div class="tags-show fr clearfix" style="width: 729px!important;">
												<div v-for="(item,index) in dynamicTags" >
				                    <span :class="{'detail-selected': addskillTagID.includes(item.id) }" @click="handlePush(item,$event)" :tag-id=index >{{item.name}} </span>
                  		  </div>
					        	</div>
