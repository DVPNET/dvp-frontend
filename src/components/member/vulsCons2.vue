	...
	<div>
		<el-table  :data="myVulslddtDatas"  style="width: 100%" class="myvul tabbg">
            ...
            <el-table-column class="hole_status_th" prop="state"  :label="$t('message.audit_status')"  width="140">
            	<template slot-scope="scope">
		        	<p v-if="scope.row.state == 0 && scope.row.reedit ==0"><i class="icon iconfont icon-shenhebijijishibenxiezi p8 ser" :title="$t('icon_title.beingAudit')"></i>{{$t('message.BeingAudit')}}</p>
		        	<p v-if="scope.row.state == 0 && scope.row.reedit ==1"><i class="icon iconfont icon-daishenhe2 p8 ser" :title="$t('icon_title.siVerified')"></i>{{$t('message.SupplementInformationVerified')}}</p>
		        	<p v-if="scope.row.state == 1"><i class="icon iconfont icon-fuxuankuang_xuanzhong p8 low" :title="$t('icon_title.approved')"></i>{{$t('message.approved')}}</p>
		        	<p v-if="scope.row.state == 2"><i class="icon iconfont icon-shalou_huaban p8 ser" :title="$t('icon_title.vfAdditional')"></i>{{$t('message.VerificationFailedAdditionalInformationRequired')}}</p>
		        	<p v-if="scope.row.state == 3"><i class="icon iconfont icon-shanchu p8 mid" :title="$t('icon_title.verificationFailed')"></i>{{$t('message.VerificationFailed')}}</p>
				</template>
            </el-table-column>
            <el-table-column class="hole_claim_th" prop="rlstatus"  :label="$t('message.vulStatus')"  width="120">
            	<template slot-scope="scope">
