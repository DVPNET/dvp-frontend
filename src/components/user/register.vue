    ...
    <div class="home-wrap">
        ...
        <div class="logAreg_boxbg" v-show="!comEmailinfo">
            ...
            <div class="logAreg_box" v-show="registerStatus" style="position: relative;">
                <div class="ac" style="position: absolute;top: -30px;left:0;right: 0;width: 100%;" v-show='resShowBMZ'>
                    <strong><i
                        class="el-icon-warning"></i>{{ $t('message.Please_save_your_password_properly') }}</strong>
                </div>
                <div class="ac" style="position: absolute;top: -30px;left:0;right: 0;width: 100%;" v-show='resShowCS'>
                    <strong><i
                        class="el-icon-warning"></i>{{ $t('message.Please_save_your_password_properly') }}</strong>
                </div>
                <h2 v-show='loginShow'>用户注册</h2>
                <h2 v-show="resShowBMZ">作为<span class="comcolor">白帽子</span>注册</h2>
                <h2 v-show="dialog_zcbq">白帽子注册</h2>
                <h2 v-show="resShowCS">作为<span class="comcolor">厂商</span>注册</h2><a @click="Role" class="fr"
                                                                                   v-show='!resSucess'
                                                                                   style="position: absolute; top:60px;right:30px;cursor: pointer;">{{ Roleswitch }}</a>
                ...
                <div v-show="resShowBMZ">
                    <el-form :model="DVP_BMZRes" v-enterToNext="true" ref="DVP_BMZRes" label-width="100px"
                             class="demo-dynamic resbox" status-icon :rules="DVP_BMZResS"
                        ...
                        <el-form-item label="" prop="pass">
                            <el-input type="password" v-model="DVP_BMZRes.pass" autocomplete="off"
                                      :placeholder="$t('message.Your_password_must_be_6_50_characters')"></el-input>
                        </el-form-item>
                        <el-form-item label="" prop="checkPass">
                            <el-input type="password" v-model="DVP_BMZRes.checkPass" autocomplete="off"
                                      placeholder="确认密码"></el-input>
                        </el-form-item>
                        <el-form-item label="" prop="yzm" class="yzmbox">
                            <el-input type="text" v-model="DVP_BMZRes.yzm"
                                      :placeholder="$t('message.input_verification_code')"
                                      @keyup.enter.native="submitForm('DVP_BMZRes')"></el-input>
                            <span class="yzmimg cp"><img :src="imgurl" alt="验证码" @click="changeCodeImg" title="点击换一张"/></span>
                        </el-form-item>
                        <el-form-item>
                            <el-checkbox v-model="checked">{{ $t('message.i_have_view_agreement') }}
                                <router-link to="/agreement" target="_blank">《{{ $t('dvp_footer.Dvp_Agreement') }}》
                                </router-link>
                            </el-checkbox>
                        </el-form-item>
                        <el-form-item>
                            <el-button type="primary" class="nextBtn btnshadow" @click="submitForm('DVP_BMZRes')"
                                       :plain="true" style="background: #2196f3 !important;color: #ffffff !important;">
                                {{ $t('dvp_head.register') }}
                            </el-button>
                        </el-form-item>
                    </el-form>
                ...
                <div v-show="resShowCS">
                    <el-form :model="DVP_CSRes" v-enterToNext="true" ref="DVP_CSRes" label-width="100px"
                             class="demo-dynamic" status-icon :rules="DVP_CSResS" :label-position="labelPosition">
                        <el-form-item prop="user" label="">
                            <el-input v-model="DVP_CSRes.user"
                                      :placeholder="$t('message.Please_input_Enterprise_Name')"></el-input>
                        </el-form-item>
                        ...
                        <el-form-item label="" prop="pass">
                            <el-input type="password" v-model="DVP_CSRes.pass" autocomplete="off"
                                      placeholder="输入密码"></el-input>
                        </el-form-item>
                        <el-form-item label="" prop="checkPass">
                            <el-input type="password" v-model="DVP_CSRes.checkPass" autocomplete="off"
                                      placeholder="确认密码"></el-input>
                        </el-form-item>
                        <el-form-item label="" prop="yzm" class="yzmbox">
                            <el-input type="text" v-model="DVP_CSRes.yzm"
                                      :placeholder="$t('message.input_verification_code')"
                                      @keyup.enter.native="submitForm('DVP_CSRes')"></el-input>
                            <span class="yzmimg cp"><img :src="imgurl" alt="验证码" @click="changeCodeImg" title="点击换一张"/></span>
                        </el-form-item>
                        <el-form-item>
                            <el-checkbox v-model="checked">{{ $t('message.i_have_view_agreement') }}
                                <router-link to="/agreement_ven" target="_blank">
                                    《{{ $t('dvp_footer.Dvp_Agreement_Ven') }}》
                                </router-link>
                            </el-checkbox>
                        </el-form-item>
                        <el-form-item>
                            <el-button type="primary" @click="submitCSForm('DVP_CSRes')" class="nextBtn btnshadow"
                                       :plain="true" style="background: #2196f3 !important;color: #ffffff !important;">
                                {{ $t('dvp_head.register') }}
                            </el-button>
                        </el-form-item>
                    ...
                    <div class="form-input">
                        <div class="my-info-edit-item clearfix">
                            ...
                            <div class="tags-select fr">
                                <el-tag :key="tag" v-for="tag in dynamic" size="medium" closable
                                        :disable-transitions="false" @close="handleClosebq(tag)">{{ tag }}
                                </el-tag>
                            </div>
                            <div class="tags-show fr clearfix">
                                <div v-for="(item,index) in dynamicTags">
                                    <span :class="{'detail-selected': dynamicTagIds.includes(item.id) }"
                                          @click="handlePush(item,$event)" :tag-id=index>{{ item.name }}</span>
                                </div>
                            </div>
                            <div style="clear: both;text-align: center;height:80px;display: inline-block;">
                                <el-button type="primary" class="nextBtn btnshadow" @click="submitSkill" :plain="true"
                                           style="background: #2196f3 !important;color: #ffffff !important;">完成
                                </el-button>
                            </div>
                        </div>
                ...
                <div v-show="resSucess" class='ac'>
                    ...
                    <el-button type="primary" class="loginWCBtn" @click="BMZuserBtn" style="display: none;">
                        <b>{{ count }}</b>进入个人中心
                    </el-button>
                </div>
                <!--厂商注册成功-->
                <div v-show="resCSSucess" class='ac'>
                    <img src="/static/image/logAres/sucessIcon.png" style="margin-top: 30px;" alt=""/><br/>
                    <p style="font-size: 16px;margin: 30px 0;">注册成功</p>
                    <el-button type="primary" class="loginWCBtn" @click="CSuserBtn" style="display:none">
                        {{ count }}进入个人中心
                    </el-button>
                </div>
            </div>
...
import comEmail from "../company/comEmail";
...
export default {
    ...
    methods: {
        ...
        submitCSForm(DVP_CSResS) {
            const paramType = this.$route.params.type || '0';
            let formData = {
