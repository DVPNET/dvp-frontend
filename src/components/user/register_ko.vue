    ...
    <div class="home-wrap">
        ...
        <div class="logAreg_boxbg" v-show="!comEmailinfo">
            ...
            <div class="logAreg_box" v-show="registerStatus" style="position: relative;">
            	...
            	<h2 v-show='loginShow'>회원가입 하기</h2>
							<h2 v-show="resShowBMZ"><span class="comcolor">화이트 해커</span></h2>
							<h2 v-show="dialog_zcbq">화이트해커</h2>
							<h2 v-show="resShowCS"><span class="comcolor">공급업체</span></h2>
							<a @click="Role" class="fr" v-show='!resSucess' style="position: absolute; top:60px;right:30px;cursor: pointer;" >{{Roleswitch}}</a>
	        	<el-steps :active="active" align-center>
				  <el-step :title="$t('message.selsctRole')"></el-step>
				  <el-step title="가입하기"></el-step>
	        	...
	        	<div class="roleBox" v-show='loginShow'>
					 ...
					 <el-button class="nextBtn btnshadow" @click="next"  type="primary" round>다음</el-button>
					 <div @click="loginBtn" class="ac">이미 계정이 있으십니까? <a href="#">바로 로그인</a></div>
				</div>
				<!--白帽子注册-->
	                ...
	                <el-form :model="DVP_BMZRes"  v-enterToNext="true"  ref="DVP_BMZRes" label-width="100px" class="demo-dynamic resbox"  status-icon  :rules="DVP_BMZResS"  :label-position="labelPosition" >
	                    <el-form-item prop="user"   label="" >
	                     ...
	                     <el-form-item prop="address"   label="" >
	                        ...
	                        <el-tooltip content="비밀번호 분실 방지하기 위해 지갑으로 로그인에 사용할 수 있습니다" placement="top">
  <el-input v-model="DVP_BMZRes.address" placeholder="이더리움 주소 (옵션)"></el-input>
							</el-tooltip>
	                    </el-form-item>
	                    ...
	                    <el-form-item label="" prop="checkPass">
	                        <el-input type="password" v-model="DVP_BMZRes.checkPass" autocomplete="off" placeholder="비밀번호 재확인"></el-input>
	                    </el-form-item>
	                    ...
	                    <el-form-item>
		                	<el-checkbox v-model="checked" class="agree-check">{{$t('message.i_have_view_agreement')}}  <br><router-link to="/agreement" target="_blank">《{{$t('dvp_footer.Dvp_Agreement')}}》</router-link></el-checkbox>
		                </el-form-item>
	                    <el-form-item>
                ...
                <div v-show="resShowCS">
                    <el-form :model="DVP_CSRes"  v-enterToNext="true"  ref="DVP_CSRes"  label-width="100px" class="demo-dynamic resbox" status-icon :rules="DVP_CSResS"  :label-position="labelPosition" >
                          ...
                          <el-form-item prop="comqyym" label="">
                              <el-input v-model="DVP_CSRes.comqyym" placeholder="기업 웹 사이트 링크를 입력하세요. (‘https://’ 필수)"></el-input>
                          </el-form-item>
                          <el-form-item prop="email" label=""  :rules="[
                              ...
                              <el-input v-model="DVP_CSRes.email" placeholder="프로젝트 측 이메일 주소를 입력하세요"></el-input>
                          </el-form-item>
                          <el-form-item prop="contact" label="">
                              <el-input v-model="DVP_CSRes.contact" placeholder="전화번호/ Wechat / telegram 계정을 입력하세요"></el-input>
                          </el-form-item>
                          <el-form-item label="" prop="pass">
                              <el-input type="password" v-model="DVP_CSRes.pass" autocomplete="off" placeholder="비밀번호를 입력하세요"></el-input>
                          </el-form-item>
                           <el-form-item label="" prop="checkPass">
                               <el-input type="password" v-model="DVP_CSRes.checkPass" autocomplete="off" placeholder="비밀번호 재확인"></el-input>
                          </el-form-item>
                          <el-form-item label="" prop="yzm" class="yzmbox">
                          ...
                          <el-form-item>
			                	<el-checkbox v-model="checked" class="agree-check">{{$t('message.i_have_view_agreement')}}<br><router-link to="/agreement_ven" target="_blank">《{{$t('dvp_footer.Dvp_Agreement_Ven')}}》</router-link></el-checkbox>
                          </el-form-item>
                          <el-form-item>
				        ...
				        <div class="my-info-edit-item clearfix">
				        	...
				        	<div class="tags-show fr clearfix">
                  			...
                  			<div style="clear: both;text-align: center;height:80px;display: inline-block;">
                  				<el-button type="primary" class="nextBtn btnshadow" @click="submitSkill"   :plain="true"  style="background: #2196f3 !important;color: #ffffff !important;">가입하기</el-button>
                  			</div>
				   		 </div>
				...
				<div v-show="resSucess" class='ac'>
					...
					<p style="font-size: 16px;margin: 30px 0;">가입 완료</p>
...
<script>
	...
	import comEmailEn from '@/components/company/comEmailKo'
	export default {
		name: 'register',
		...
		var validatePass = (rule, value, callback) => {
	        ...
	        if (value === '') {
	        	...
	        	callback(new Error('비밀번호는 문자,숫자,특수기호가 모두 포함되어야 합니다'));
	        }else {
	    ...
	    var validateyzm = (rule, value, callback) => {
	    	if (value === '') {
				callback(new Error('인증번호를 입력하여 주세요!'))
			}
	      	callback()
	    };
		...
		return {
			...
			DVP_BMZResS: {
				user: [
					{validator: validateUser, trigger: 'blur', message: "닉네임을 입력하세요"},
...
.agree-check .el-checkbox__input {
	position: relative; 
	top: -20px; 
	left: 0;
}
