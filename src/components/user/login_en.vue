<template>
	<div class="home-wrap">
		...
		<div class="logAreg_boxbg" v-show="!comEmailinfo">
			<div
				class="logAreg_box wrapshadow"
				style="position: relative"
				v-show="!passEmailinfo"
			>
				<h2 v-show="loginShow">{{ $t("message.userLogin") }}</h2>
				<h2 v-show="loginShowBMZ">
					Login as a <span class="comcolor">White Hat</span
					><a @click="Role" class="fr Roleswitch">{{ Roleswitch }}</a>
				</h2>
				<h2 v-show="loginShowCS">
					Login as a <span class="comcolor">Vendor</span
					><a @click="Role" class="fr Roleswitch">{{ Roleswitch }}</a>
				</h2>
				...
				<div v-show="loginShowBMZ">
					...
					>
						...
						<el-form-item label="" prop="pass">
							<el-input
								type="password"
								v-model="DVP_BMZLog.pass"
								autocomplete="off"
								placeholder="Input Possword"
							></el-input>
						</el-form-item>
						<el-form-item label="" prop="yzm" class="yzmbox">
							<el-input
								type="text"
								v-model="DVP_BMZLog.yzm"
								:placeholder="$t('message.input_verification_code')"
								@keyup.enter.native="submitForm('DVP_BMZLog')"
							></el-input>
							<span class="yzmimg cp"
								><img
									:src="imgurl"
									alt="验证码"
									@click="changeCodeImg"
					...
					<div id="demo" style="margin-bottom: 50px">
						<el-button
							...
							><svg class="icon metamaskIcon" aria-hidden="true">
								<use xlink:href="#icon-metamask1"></use></svg
							>{{ loginmetaState }}</el-button
						>
				...
				<div v-show="loginShowCS">
					...
					>
						<el-form-item prop="user" label="">
							<el-input
								v-model="DVP_CSLog.user"
								:placeholder="$t('message.Please_Input_CompanyName')"
							></el-input>
						</el-form-item>
						<el-form-item label="" prop="pass">
							<el-input
								type="password"
								v-model="DVP_CSLog.pass"
								autocomplete="off"
								placeholder="Input Possword"
							></el-input>
						</el-form-item>
						<el-form-item label="" prop="yzm" class="yzmbox">
							<el-input
								type="yzm"
								v-model="DVP_CSLog.yzm"
								:placeholder="$t('message.input_verification_code')"
								@keyup.enter.native="submitCSForm('DVP_CSLog')"
							></el-input>
							<span class="yzmimg cp"
								><img
									:src="imgurl"
						...
						<el-form-item>
							<p
								class="cp"
								@click="respass"
								style="padding: 0 15px; color: #2196f3"
							>
								Forget Password？
							</p>
							<el-button
				...
				<div v-show="resSucess" class="ac">
					<img
						src="/static/image/logAres/sucessIcon.png"
						style="margin-top: 30px"
						alt=""
					/><br />
					<p style="font-size: 16px; margin: 30px 0">
						{{ $t("message.loginSuccess") }}
					</p>
					<el-button
						type="primary"
						class="loginWCBtn"
		...
		<!--metamask-->
		<el-dialog title="提示" :visible.sync="dialog_metaMerr" width="432px">
			<p class="ac">Please install MetaMask first</p>
			<a
...
<script>
// import VueMetamask from "@/components/user/vueMetamask";
// import homeBanner from "@/components/Home/homeBanner";
// import passEmailEn from "@/components/company/passEmailEn";
// import comEmailEn from "@/components/company/comEmailEn";
import { mapState, mapMutations } from "vuex";
import VueMetamask from "./VueMetamask";
import homeBanner from "../Home/homeBanner";
import passEmailEn from "../company/passEmailEn";
import comEmailEn from "../company/comEmailEn";
export default {
	name: "login",
	...
	data() {
		return {
			...
			loginmetaState: "Login with MetaMask",
	...
	methods: {
		...
		metamaskBtn() {
			var that = this;
			if (
				this.metaMaskType == "NO_INSTALL_METAMASK" &&
				this.metaMaskAddress == ""
			) {
				this.dialog_metaMerr = true;
			} else if (
				this.metaMaskType == "NO_LOGIN" &&
				this.metaMaskAddress == ""
			) {
				this.$message.error("Please login to Metamask first");
			} else {
				this.loadingmetaBtn = true;
				this.loginmetaState = "logging in...";
				//metaMask nonce
				this.$ajax
					.BMZmetaMnonce({ receipt: this.metaMaskAddress })
					.then((res) => {
						if (res.errcode === 1) {
							...
							web3.personal.sign(
								...
								function (res, sigmsg) {
									if (res == null) {
											...
											.then((res) => {
												if (res.errcode === 1) {
													...
													setTimeout(() => {
														...
														that.loginmetaState = "Login with MetaMask";
													}, 300);
										...
										that.captcha();
										that.$message.error(
											"You canceled the signature and failed to login"
										);
										setTimeout(() => {
											that.loadingmetaBtn = false;
											that.loginmetaState = "Login with MetaMask";
						...
						if (!this.timer) {
							...
							this.timer = setInterval(() => {
								if (this.count > 0 && this.count <= TIME_COUNT) {
									...
									window.localStorage.setItem("tokenType", 1);
									this.$router.push({
										path: this.$route.query.redirect || "/membox/member/vuls",
									});
									//白帽子个人信息
...
.metamaskBtn {
	...
	-webkit-box-shadow: 0 5px 40px 0 rgba(246, 133, 27, 0.1);
	-moz-box-shadow: 0 5px 40px 0 rgba(246, 133, 27, 0.1);
	box-shadow: 0 5px 40px 0 rgba(246, 133, 27, 0.1);
}
.metamaskBtn:hover {
	border: 1px solid #f6851b !important;
	color: #ffffff !important;
...
.home-wrap .DVP-header-wrap .el-menu-item,
.home-wrap .DVP-header-wrap .el-submenu__title {
	color: #ffffff !important;
}
...
.home-wrap .DVP_home_header li,
.home-wrap .DVP_home_header .home_headup .el-submenu__title {
