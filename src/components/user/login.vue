    ...
    <div class="home-wrap">
        ...
        <div class="logAreg_boxbg" v-show="!comEmailinfo">
            <div class="logAreg_box  wrapshadow" style="position: relative;" v-show="!passEmailinfo">
                ...
                <h2 v-show="loginShowBMZ">
                    {{ $t('Login.As') }}
                    <span class="comcolor">
                        {{ $t('Login.WhiteHat') }}
                    </span>
                    {{ $t('Login.Login') }}
                    <a @click="Role" class="fr Roleswitch">
                        {{ Roleswitch }}
                    </a>
                </h2>
                <h2 v-show="loginShowCS">
                    {{ $t('Login.As') }}
                    <span class="comcolor">
                       {{ $t('Login.Vendor') }}
                    </span>
                ...
                <div class="roleBox" v-show='loginShow'>
                    ...
                    <el-button class="nextBtn btnshadow" @click="next" type="primary" round>{{ $t('message.next') }}
                    </el-button>
                </div>
                <!--登录-->
                <div v-show="loginShowBMZ">
                    <el-form :model="DVP_BMZLog" v-enterToNext="true" ref="DVP_BMZLog" label-width="100px"
                             class="demo-ruleForm" status-icon :rules="DVP_BMZLogS" :label-position="labelPosition">
                        <el-form-item prop="user" label="">
                            <el-input type="text" v-model="DVP_BMZLog.user"
                                      :placeholder="$t('message.InputNickname')"></el-input>
                        </el-form-item>
                        <el-form-item label="" prop="pass">
                            <el-input type="password" v-model="DVP_BMZLog.pass" autocomplete="off"
                                      :placeholder="$t('Login.EnterPassword')"></el-input>
                        </el-form-item>
                        <el-form-item label="" prop="yzm" class="yzmbox">
                            <el-input type="text" v-model="DVP_BMZLog.yzm"
                                      :placeholder="$t('message.input_verification_code')"
                                      @keyup.enter.native="submitForm('DVP_BMZLog')"></el-input>
                            <span class="yzmimg cp"><img :src="imgurl" :alt="$t('Login.VerificationCode')"
                                                         @click="changeCodeImg"
                                                         :title="$t('Login.ClickChange')"/></span>
                        </el-form-item>
                        <el-form-item>
                            <el-button type="primary" @click="submitForm(DVP_BMZLog)" class="nextBtn btnshadow"
                                       :plain="true"
                                       style="background: #2196f3 !important;color: #ffffff !important;"
                                       :loading="loadingBtn">{{ loginState }}
                            </el-button>
                        </el-form-item>
                    </el-form>
                    <div id="demo" style="margin-bottom: 50px;">
                        <el-button class="metamaskBtn" round
                                   :loading="loadingmetaBtn"
                                   :plain="true"
                                   @click="metamaskBtn">
                            <svg class="icon metamaskIcon" aria-hidden="true">
                                <use xlink:href="#icon-metamask1"></use>
                            </svg>
                            {{ loginmetaState }}
                        </el-button>
                        <vue-metamask
                ...
                <div v-show="loginShowCS">
                    <el-form :model="DVP_CSLog" v-enterToNext="true" ref="DVP_CSLog" label-width="100px"
                             class="demo-ruleForm" status-icon :rules="DVP_CSLogS" :label-position="labelPosition">
                        <el-form-item prop="user" label="">
                            <el-input v-model="DVP_CSLog.user"
                                      :placeholder="$t('message.Please_Input_CompanyName')"></el-input>
                        </el-form-item>
                        <el-form-item label="" prop="pass">
                            <el-input type="password" v-model="DVP_CSLog.pass" autocomplete="off"
                                      :placeholder="$t('Login.EnterPassword')"></el-input>
                        </el-form-item>
                        <el-form-item label="" prop="yzm" class="yzmbox">
                            <el-input type="yzm" v-model="DVP_CSLog.yzm"
                                      :placeholder="$t('message.input_verification_code')"
                                      @keyup.enter.native="submitCSForm('DVP_CSLog')"></el-input>
                            <span class="yzmimg cp"><img :src="imgurl"
                                                         :alt="$t('Login.VerificationCode')"
                                                         @click="changeCodeImg"
                                                         :title="$t('Login.ClickChange')"/></span>
                        </el-form-item>
                        <el-form-item>
                            <p class="cp" @click="respass" style="padding: 0 15px;color: #2196f3;">
                                {{ $t('Login.ForgotPassword') }}？</p>
                            <el-button type="primary"
                                       @click="submitCSForm(DVP_CSLog)"
                                       class="nextBtn btnshadow"
                ...
                <div v-show="resSucess" class='ac'>
                    ...
                    <el-button type="primary" class="loginWCBtn" @click="BMZuserBtn" style="display: none;">
                        <span v-html="$t('Login.CountDown')"></span>
                    </el-button>
                </div>
            ...
            <!--邮箱信息-->
            <passEmail v-show="passEmailinfo"></passEmail>
        </div>
        <!--邮箱信息-->
        <comEmail v-show="comEmailinfo" :userEmail="userDLEmail"></comEmail>
        <!--metamask-->
        <el-dialog
            :title="$t('ComReward.Tips')"
            :visible.sync="dialog_metaMerr"
            width="432px">
            <p class="ac">{{ $t('Login.NoMetamask') }}</p>
            <a href="https://chrome.google.com/webstore/detail/nkbihfbeogaeaoehlefnkodbefgpgknn" target="_blank">
                <el-button class="nextBtn btnshadow" style="background: #2196f3 !important;color: #ffffff !important;">
                    {{ $t('Login.DownloadMetamask') }}
                </el-button>
            </a>
        </el-dialog>
    </div>
...
import passEmail from "../company/passEmail";
import comEmail from "../company/comEmail";

export default {
    name: 'login',
    components: {homeBanner, comEmail, passEmail, VueMetamask},
    data() {
        return {
            passEmailinfo: false,
            loadingBtn: false,
    ...
    methods: {
        ...
        metamaskBtn() {
            ...
            if (this.metaMaskType == "NO_INSTALL_METAMASK" && this.metaMaskAddress == '') {
                ...
                this.$message.error("请登录您的Metamask账号");
            } else {
                this.loadingmetaBtn = true
                this.loginmetaState = "登陆中..."
                //metaMask nonce
                        ...
                        if (res.errcode === 1) {
                            ...
                            web3.personal.sign(web3.fromUtf8(`I am signing my one-time nonce: ${res.data.nonce}`), web3.eth.coinbase, function (res, sigmsg) {
                                if (res == null) {
                                    that.$ajax.BMZmetaMnonceAuth({'receipt': that.metaMaskAddress, 'signature': sigmsg})
                                        .then(res => {
                                            if (res.errcode === 1) {
                                                ...
                                                if (!that.timer) {
                                                    ...
                                                    that.timer = setInterval(() => {
                                                        if (that.count > 0 && that.count <= TIME_COUNT) {
                                                            ...
                                                            //白帽子个人信息
                                                            that.$ajax.BMZinfo({'language': 'zh_CN'})
                                                                .then(res => {
                                                                    if (res.errcode === 1) {
                                                ...
                                                that.captcha()
                                                that.$message.error("无此用户，请注册");
                                                setTimeout(() => {
                                                    that.loadingmetaBtn = false
                                                    that.loginmetaState = "Metamask 一键登录"
                                                }, 300);
                                    ...
                                    that.captcha()
                                    that.$message.error("您取消了标签，无法登陆");
                                    setTimeout(() => {
                                        that.loadingmetaBtn = false
                                        that.loginmetaState = "Metamask 一键登录"
                                    }, 300);
