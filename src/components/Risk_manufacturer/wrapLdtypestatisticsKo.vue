...
                       this.months = resYear[Object.keys(resYear)[1]].map(item=>item);
		    	...
		    	this.$ajax.ComVulTypeMonth({year:'2019',month: '00','language':'en_US'})
		    	.then(res =>{
		    		if(res.errcode === 1){
		    			_this.fxcsData = res.data.targetTypeMonth
		    			for(var i=0;i<res.data.targetTypeMonth.length;i++){
				      	 	this.datas1.push({value:_this.fxcsData[i].num, name:_this.fxcsData[i].name_en})
				      	 }
			...
			rankingsSelectYear(year){
				...
				var _this=this
		     	this.$ajax.ComVulTypeMonth({year:this.value1,month:"00",'language':'en_US'})
		    	.then(res =>{
		    		if(res.errcode === 1){
		    			_this.fxcsData = res.data.targetTypeMonth
		    			for(var i=0;i<res.data.targetTypeMonth.length;i++){
		    ...
		    rankSelect(){
		    	var _this=this
		    	this.$ajax.ComVulTypeMonth({year:this.value1,month: this.value2,'language':'en_US'})
		    	.then(res =>{
		    		if(res.errcode === 1){
		    			_this.fxcsData = res.data.targetTypeMonth
		    			for(var i=0;i<res.data.targetTypeMonth.length;i++){
