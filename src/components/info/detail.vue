<template>
		<div  class="hole-detailed-wrap" style="margin-top: 80px;">
					<div class="fr share" v-show="fenxiang">
						    <span>分享到 &nbsp;</span>
						    <el-popover
							  placement="bottom"
							  width="100"
							  trigger="hover">
							  <img src='/static/image/wxewm.jpg' class="wechat">
							  <span slot="reference"  class="wechat-share"><img src="/static/image/weixin-at.png"></span>
							</el-popover>
							<el-tooltip  placement="top" popper-class="aa"></el-tooltip>
						    <span class="weibo-share"><a><img src="/static/image/weibo-at.png"></a></span>
						    <vue-q-art :config="config" :downloadButton="downloadButton"></vue-q-art>
					</div>
					<div class="openvul" v-if="dialogMyVul_data.is_open==1">
  			   			<div class="al">
					...
					<el-breadcrumb separator-class="el-icon-arrow-right">
						<el-breadcrumb-item :to="{ path: '/lddt' }">{{$t('dvp_head.lddt')}}</el-breadcrumb-item>
						<el-breadcrumb-item :to="{ path: '/lddt' }">{{$t('message.vulnerabilityDetail')}}</el-breadcrumb-item>
						<el-breadcrumb-item>{{dialogMyVul_data.vulSign}}</el-breadcrumb-item>
					</el-breadcrumb>
					<div class="hole-detailed hole-detailed1" style="position: relative;">
						    ...
						    <div style="position: absolute;right: 15px;top: 15px;" v-if="dialogMyVul_data.is_fix == 1 || dialogMyVul_data.is_open == 1"><img src="/static/image/common/xiufuSucess.png" alt=""/></div>
					</div>
					<!--等待审核-->
					<div class="hole-header hole-header1" v-if="dialogMyVul_data.state == 0">
						<div class="hole-headerTop">
							...
							<div class="hole-progress fl"  align-center>
								  	<el-steps :active="2" >
									  	<el-step  :title="$t('message.ToBeVerified')" :description="dialogMyVul_data.write_time"></el-step>
						 ...
						 <div class="hole-status1">
						 	<div>
						 		<p v-for="val in dialogMyVul_data.log">
						 			<b></b><span v-for="(vals,key) in val">{{key}}&nbsp;&nbsp;&nbsp;{{vals}}</span></p>
						 		</p>
						 	</div>							    
						 </div>
					</div>
					<!--等待认领/7{{$t('message.day')}}-->
							...
							<div class="hole-proleft fl">
								...
								<div class="hole-proleftcen">
									<span>{{$t('message.ToBeClaimed')}}</span>
					    	 		<b>{{dialogMyVul_data.remain_time}}</b>
								</div>
								<div class="hole-proleftbom">
									<div v-if="dialogMyVul_data.state==0">
										<p v-if="dialogMyVul_data.level == 3">{{$t('message.vulnerability_level')}}：<span class="com-dot-lv ser-lv"></span> 严重</p>
							        	<p v-if="dialogMyVul_data.level == 2">{{$t('message.vulnerability_level')}}：<span class="com-dot-lv high-lv"></span>高危 </p>
							        	<p v-if="dialogMyVul_data.level == 1">{{$t('message.vulnerability_level')}}：<span class="com-dot-lv mid-lv"></span> 中危</p>
							        	<p v-if="dialogMyVul_data.level == 0">{{$t('message.vulnerability_level')}}：<span class="com-dot-lv low-lv"></span> 低危</p>
									</div>
									<div v-if="dialogMyVul_data.state!=0">
										<p v-if="dialogMyVul_data.dvp_level == 3">{{$t('message.vulnerability_level')}}：<span class="com-dot-lv ser-lv"></span> 严重</p>
							        	<p v-if="dialogMyVul_data.dvp_level == 2">{{$t('message.vulnerability_level')}}：<span class="com-dot-lv high-lv"></span>高危 </p>
							        	<p v-if="dialogMyVul_data.dvp_level == 1">{{$t('message.vulnerability_level')}}：<span class="com-dot-lv mid-lv"></span> 中危</p>
							        	<p v-if="dialogMyVul_data.dvp_level == 0">{{$t('message.vulnerability_level')}}：<span class="com-dot-lv low-lv"></span> 低危</p>
									</div>									
								</div>
					...
					<!--等待补充/审核未通过-->
					<div class="hole-header hole-header3" v-if="dialogMyVul_data.state == 2 && dialogMyVul_data.reedit == 0">
						<div class="hole-headerTop">
							<div class="hole-proleft fl">
								<p>{{$t('message.vulSerialNumber')}}：{{dialogMyVul_data.vulSign}}</p>
								<div class="hole-proleftcen">
								...
								<div class="hole-proleftbom ac">
									<el-button type="primary" @click="dialogMyBJVulBtn()">{{$t('message.SupplementInformation')}}</el-button>
								</div>
								
							</div>
							<div class="hole-progress fl">
									<div class="hole-proHead">{{$t('message.hours24rejectedAutomatically')}}</div>
									<div class="progress_cpytime"><span>{{$t('message.ReportSupplemented')}}</span><br />7{{$t('message.hour')}}</div>
					...
					<div class="hole-header hole-header1" v-if="dialogMyVul_data.state == 2 && dialogMyVul_data.reedit == 1">
						<div class="hole-headerTop">
							<div class="hole-proleft fl">
								<p>{{$t('message.vulSerialNumber')}}：{{dialogMyVul_data.vulSign}}</p>
								<div class="hole-proleftcen">
						...
						<div class="hole-headerTop">
							...
							<div class="hole-progress fl">
								  	<el-steps :active="3"  align-center>
									  	<el-step  :title="$t('btn.sub_vul')" :description="dialogMyVul_data.write_time"></el-step>
									  	<el-step  :title="$t('message.VerificationPassed')"  :description="dialogMyVul_data.update_time"></el-step>
									  	<el-step  :title="$t('message.ClaimedbyVendor')"  :description="dialogMyVul_data.company_time"></el-step>
									</el-steps>
							</div>
						</div>						
						<div class="clearfix"></div>
					...
					<!--完成/DVP认领 == 厂商未认领-->
					<div class="hole-header hole-header7" v-if="dialogMyVul_data.state == 1 && dialogMyVul_data.company_state == 3">
						<div class="hole-headerTop">
							<div class="hole-proleft fl">
								<p>{{$t('message.vulSerialNumber')}}：{{dialogMyVul_data.vulSign}}</p>
								<div class="hole-proleftcen hole-proleftcen5">
							...
							<div class="hole-progress fl">
								<div class="hole-proHead">{{$t('message.RemarksevenDaysCSnoDVPClaimLoopholeAutomatically')}}</div>
								  	<el-steps :active="4"  align-center>
									  	<el-step  :title="$t('btn.sub_vul')" :description="dialogMyVul_data.write_time"></el-step>
									  	<el-step  :title="$t('message.VerificationPassed')"  :description="dialogMyVul_data.update_time"></el-step>
									  	<el-step  :title="$t('message.The_manufacturer_failed_to_identify_the_vul')"  :description="dialogMyVul_data.company_time"></el-step>
						...
						<div class="hole-headerTop">
							...
							<div class="hole-progress fl">
								  	<el-steps :active="4"  align-center>
									  	<el-step  :title="$t('btn.sub_vul')" :description="dialogMyVul_data.write_time"></el-step>
									  	<el-step  :title="$t('message.VerificationPassed')"  :description="dialogMyVul_data.update_time"></el-step>
									  	<el-step  :title="$t('message.VendorFailedClaimLoophole')"  :description="dialogMyVul_data.company_time"></el-step>
									  	<el-step  :title="$t('message.ClaimedDVP')"  :description="dialogMyVul_data.company_time"></el-step>
									</el-steps>
							</div>
						</div>						
					...
					<div class="hole-detailed">
						    ...
						    <div class="clearfix simditor" style="border: none;width: 100%;">
						      <div class="fl simditor-body" style="" v-html="dialogMyVul_data.description_open"></div>
						    </div>
					</div>
					...
					<div class="hole-detailed" v-if="dialogMyVul_data.is_open == 1 && dialogMyVul_data.repair">
							 <p>修复建议</p>
							 <div class="clearfix simditor" style="border: none;width: 100%;">
						      <div class="fl simditor-body" style="" v-html="dialogMyVul_data.repair"></div>
						    </div>
					</div>
				 	<!-- 申请入驻弹窗 -->
					<el-dialog
						  ...
						  width="500px">
						  	<div class="dialogTXCon">
							    ...
							    <el-button type="primary" class="common-btn" @click="dialogApply = false">{{$t('mesage.sure')}}</el-button>
							</div>
					</el-dialog> 
					<!-- 打赏弹出 -->
					<el-dialog
					  title=""
					  :visible.sync="dashangShow"
					  width="720px">
					 <div style="width: 720px;text-align: center;">
						    <div class="apply-safeguards-pop-up common-pop-up">
						      <p class="apply-title" style="margin-bottom: 30px;">打赏金额
						      	<el-tooltip placement="bottom">
								</el-tooltip></p>
						      <input type="text" placeholder="输入打赏DVP金额"  id="order" class="form-control-inp" style="width: 80%;margin-bottom: 100px;" v-model="rewardPrice">
						      <div style="clear: both;"></div>
						      <el-button type="primary" @click="rewardPublic" :plain="true" class="btnshadow wrapBtn">{{$t('message.PaymentCompleted')}}</el-button>
						    </div>
					</div>
				</el-dialog>
				<!-- 充值成功 -->
				...
				<el-dialog
					...
					<!-- 点赞  打赏 -->
					<div style="text-align:center;margin-top:20px" v-show = " dialogMyVul_data.is_open == 1" >
						<img v-if = "dialogMyVul_data.is_click == 1"  src="static/image/home/yidianzan.png" alt="已点赞"  width="50px" height="50px" />
						<img v-if = "dialogMyVul_data.is_click == 0" src="static/image/home/dianzan.png" alt="点赞" @click="clickPraise" width="50px" height="50px" />
						<p> 已有（<span>{{dialogMyVul_data.praise_num}}</span>）人点赞</p>

						<!-- <img src="static/image/home/dashang.png" alt="打赏" @click="clickReward" /> -->
					</div>
...
<script>
	...
	export default {
		...
		methods: { 
			initData(){
				...
				this.lang=localStorage.getItem('locale')
				 if(this.lang=='zh'){
					 ...
					 if(token && this.tokenType != 2){
						this.$ajax.ComPublicVulInfo({'id':id,'language':'zh_CN'})
						.then(res => {
							_this.dialogMyVul_data = res.data.info;
						})
					 }
					 else{
						this.$ajax.ComPublicVulInfoNotToken({'id':id,'language':'zh_CN'})
						.then(res => {
							_this.dialogMyVul_data = res.data.info;
						})
					}
					...
					if(token && this.tokenType != 2){
						this.$ajax.ComPublicVulInfo({'id':id,'language':'en_US'})
						.then(res => {
							_this.dialogMyVul_data = res.data.info;
						})
					}else{
						this.$ajax.ComPublicVulInfoNotToken({'id':id,'language':'en_US'})
						.then(res => {
							_this.dialogMyVul_data = res.data.info;
						})
					}
			...
			dialogMyBJVulBtn(){
    	  		this.$router.push({ path:"/edit?id="+ this.dialogMyVul_data.id})
    	   },
			ApplyBth(id){
				var _this = this; 
			...
			clickPraise(){
				if(this.tokenType != 2){
					let id = this.$route.query.id
					var _this = this; 
					this.$ajax.publicVulCilckPraise({'id': id})
					.then(res => {
						if(res.errcode === 1){
							_this.dialogMyVul_data.praise_num++;
							_this.dialogMyVul_data.is_click = 1;
						}else{
							_this.$message.error(res.errmsg);
						}
					})
				}else{
					this.$message.error('厂商该功能尚未开启');
				}
			},
			clickReward(){
			...
			rewardPublic(){
				if(this.tokenType != 2){
					let id = this.$route.query.id
					var _this = this; 
					this.$ajax.publicVulCilckReward({'id': id,'dvp':this.rewardPrice})
					.then(res => {
						if(res.errcode === 1){
...
<style>
	...
	.hole-proleft .hole-proleftcen b{
		font-size: 12px;
		font-weight: 400;
		padding-left: 20px;
		color: #8E9EAE;
